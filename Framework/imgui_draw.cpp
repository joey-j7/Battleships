...
ImDrawListSharedData::ImDrawListSharedData()
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(CircleVtx12); i++)
    {
        ...
        CircleVtx12[i] = ImVec2(cos(a), sin(a));
    }
}
...
void ImDrawList::PathArcTo(const ImVec2& centre, float radius, float a_min, float a_max, int num_segments)
{
    ...
    for (int i = 0; i <= num_segments; i++)
    {
        ...
        _Path.push_back(ImVec2(centre.x + cos(a) * radius, centre.y + sin(a) * radius));
    }
}
...
{
	  int data_size = 0;
	  char* c = LoadFile(filename, data_size);
    ...
    return AddFontFromMemoryTTF((void*)c, data_size, size_pixels, &font_cfg, glyph_ranges);
}
